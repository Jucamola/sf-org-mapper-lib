...
export async function queryHomePageComponents(conn: Connection): Promise<OrgMetadataMap> {
  const homePageComponents = await conn.tooling.query(
    'SELECT Id, Name, NamespacePrefix, ManageableState, CreatedDate, LastModifiedDate FROM HomePageComponent',
    {
      autoFetch: true,
    }
  );
  ...
  return new Map(
    homePageComponents.records.map((record) => [
      record.Id as string,
      {
